namespace Husa.Quicklister.Abor.Api.Tests.Plan
{
    ...
    public class PlanNotesControllerTests
    {
        private readonly Mock<IPlanNotesService> planNotesService;
        private readonly Mock<ILogger<PlanNotesController>> logger;
        private readonly ApplicationServicesFixture fixture;

        public PlanNotesControllerTests(ApplicationServicesFixture fixture)
        {
            this.planNotesService = new Mock<IPlanNotesService>();
            this.fixture = fixture ?? throw new ArgumentNullException(nameof(fixture));
            this.logger = new Mock<ILogger<PlanNotesController>>();
            this.Sut = new PlanNotesController(
                this.planNotesService.Object,
                this.logger.Object,
                this.fixture.Mapper);
        }

        public PlanNotesController Sut { get; set; }
        ...
        public async Task GetNotes_NotesFound_Success()
        {
            ...
            this.planNotesService.Setup(m => m.GetNotes(It.Is<Guid>(x => x == entityId)))
        ...
        public async Task CreateNotes_NotesAdded_Success()
        {
            // Arrange
            var entityId = Guid.NewGuid();
            var simpleNote = TestModelProvider.GetNoteRequest(entityId);

            this.planNotesService.Setup(m => m.CreateNote(It.IsAny<Guid>(), It.IsAny<string>(), It.IsAny<string>()))
                .Verifiable();
        ...
        public async Task GetNoteById_NoteFound_Success()
        {
            ...
            this.planNotesService.Setup(m => m.GetNote(It.Is<Guid>(x => x == entityId), It.Is<Guid>(x => x == noteId)))
        ...
        public async Task DeleteById_NoteDeleted_Success()
        {
            ...
            this.planNotesService.Setup(x => x.DeleteNote(It.Is<Guid>(x => x == entityId), It.IsAny<Guid>())).Verifiable();

            // Act and Assert
            Assert.IsAssignableFrom<OkResult>(await this.Sut.DeleteById(entityId, noteId));
            this.planNotesService.Verify(x => x.DeleteNote(It.Is<Guid>(x => x == entityId), It.Is<Guid>(x => x == noteId)), Times.Once);
        }
    }
}
