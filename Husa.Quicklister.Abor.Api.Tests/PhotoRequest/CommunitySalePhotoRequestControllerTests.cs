namespace Husa.Quicklister.Abor.Api.Tests.PhotoRequest
{
    ...
    public class CommunitySalePhotoRequestControllerTests
    {
        private readonly Mock<ICommunityPhotoService> communitySaleService;
        private readonly Mock<ILogger<SaleCommunityPhotoRequestsController>> logger;

        public CommunitySalePhotoRequestControllerTests()
        {
            this.communitySaleService = new Mock<ICommunityPhotoService>();
            this.logger = new Mock<ILogger<SaleCommunityPhotoRequestsController>>();
            this.Sut = new SaleCommunityPhotoRequestsController(this.communitySaleService.Object, this.logger.Object);
        }

        public SaleCommunityPhotoRequestsController Sut { get; set; }
        private static PropertyType PhotoType => PropertyType.Community;
        ...
        public async Task GetAsync_ResourcesFound_Success()
        {
            ...
            this.communitySaleService.Setup(m => m.GetAsync(It.IsAny<Guid>(), photoRequestFilter))
        ...
        public async Task GetByIdAsync_PhotoFound_Success()
        {
            ...
            this.communitySaleService.Setup(m => m.GetByIdAsync(It.Is<Guid>(x => x == entityId), It.Is<Guid>(x => x == photoRequestId)))
                .ReturnsAsync(response)
                .Verifiable();
        ...
        public async Task CreateAsync_PhotoRequestAdded_Success()
        {
            ...
            this.communitySaleService.Setup(m => m.CreateAsync(It.IsAny<Guid>(), It.IsAny<Request.PhotoRequest>()))
                .Verifiable();
            ...
            this.communitySaleService.Verify(x => x.CreateAsync(It.Is<Guid>(x => x == communityId), It.Is<Request.PhotoRequest>(m => m == photoRequest)), Times.Once);
        }
        ...
        public async Task DeleteById_PhotoRequestDeleted_Success()
        {
            ...
            this.communitySaleService.Setup(x => x.DeleteByIdAsync(It.Is<Guid>(x => x == communityId), It.Is<Guid>(x => x == photoRequestId))).Verifiable();

            // Act and Assert
            Assert.IsAssignableFrom<OkResult>(await this.Sut.DeleteById(communityId, photoRequestId));
            this.communitySaleService.Verify(x => x.DeleteByIdAsync(It.Is<Guid>(x => x == communityId), It.Is<Guid>(x => x == photoRequestId)), Times.Once);
        }
    }
}
